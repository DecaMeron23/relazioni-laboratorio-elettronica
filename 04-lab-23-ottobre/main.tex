...
L'inverter è un circuito digitale in cui l'uscita si trova al livello opposto dell'ingresso \autoref{tab:inverter-tabella-verita}. Per testarne il funzionamento è stato realizzato il circuito in \autoref{fig:inverterStatica}:
...
Per farlo, è stato forzato uno stato logico 0, o 1, all’ingresso dell’inverter e in uscita è stata posta una resistenza con in serie un diodo LED. Questo fa si che con uscita logica 1, il LED risulti acceso e con uscita logica 0 risulti spento. % come mostrato in \autoref{fig:statica}.
...
Successivamente andando a modificare il circuito come in \autoref{fig:inverter-dinamico}, è stato possibile verificare la caratteristica dinamica: collegando l’ingresso a un generatore di forme d’onda e fornendo un segnale ad onda quadra con un’ampiezza di $5\,\mathrm{V}_{pp}$ e un offset di $2.5\,\mathrm{V}$, è possibile osservare la negazione in uscita simulando la commutazione di un bit tra 0 e 1.
...
Il latch rappresenta una cella di memoria elementare, in grado di mantenere lo stato logico fissato anche dopo la rimozione del segnale d'ingresso.
...
Questo circuito può generare segnali fino $6\,\mathrm{MHz}$, ma a frequenze così elevate, il segnale in uscita non risulta essere un'onda quadra \autoref{fig:a-oscillatore-ring-oscillator}. Quindi per migliorarne la forma si inseriscono due inverter in cascata all'uscita, come in \autoref{fig:ring-oscillator-schematico-miglirato}. Il segnale ricavato si trova a \autoref{fig:b-oscillatore-ring-oscillator}.
...
Per poter modificare la frequenza di oscillazione si può utilizzare il circuito a \autoref{fig:ring-oscillator-schematico-freq}. Modificando i valori di una resistenza si possono ottenere diverse frequenze come mostrato in \autoref{fig:ring-oscillator-oscillare} e \autoref{fig:ring-oscillator-plot-freq-r}.
...
Uno stadio di divisione è costituito da un Flip-Flop tipo D \autoref{fig:divisore-freq-fli-flop-d}. Quest'ultimo rispetta la tabella di verità \ref{tab:divisore-frequenza-flip-flop-d-tabella-verita}. La particolarità di questo circuito è che l'uscita Q (e quindi anche il suo negato $\overline{\mathrm{Q}}$) è sincronizzata sul fronte di salita di un segnale di \textit{clock}.  
...
\begin{itemize}
	...
	\item Al successivo fronte di salita, dato che $D = \overline{Q} = 0$, si ottiene che $Q$ viene posta a zero. Si veda \autoref{fig:divisore-frequenza-esempio} al tempo $t=\,$\SI{20}{\milli\second}.
\end{itemize}
In questo modo, a seguito di due periodi del clock, l'uscita $Q$ ha eseguito un solo periodo. La prova effettuata è mostrata a figura \autoref{fig:divisore-frequenza-oscilloscopio}.
\begin{figure}[h]
	\centering
